...
const API_UNAVAILABLE_MESSAGE = `Catalog API is unavailable. From the repo root run pnpm dev:no-docker (or pnpm dev if Docker is running) to start the API on port ${BMV_DEV_API_PORT} (web on ${BMV_DEV_SITE_URL}).`;
